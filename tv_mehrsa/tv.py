...
class TV_ISO():
    def __init__(self, device):
        self.filters1 = torch.Tensor([[[[1., -1.]]]]).to(device).float()
        self.filters2 = torch.Tensor([[[[1.], [-1.]]]]).to(device).float()
...
class prox_tv_iso():
    ...
    def eval(self, y, niter, lmbda, h=1, verbose=False, stop=-1):
        v_k = torch.zeros((1, 2, y.size(2)+2, y.size(3)+2), requires_grad=False, device=self.device).float()
        u_k = torch.zeros((1, 2, y.size(2)+2, y.size(3)+2), requires_grad=False, device=self.device).float()
