...
def PSNR(y_true, y_pred):
    max_pixel = 255.0
    y_pred = tf.clip_by_value(tf.cast(y_pred, tf.float32), 0.0, max_pixel)
    y_true = tf.clip_by_value(tf.cast(y_true, tf.float32), 0.0, max_pixel)
    mse = tf.reduce_mean(tf.square(y_true - y_pred))
    psnr = 20 * tf_log10(max_pixel / tf.sqrt(mse))
    return psnr
